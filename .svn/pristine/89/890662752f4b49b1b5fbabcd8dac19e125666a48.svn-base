...
public class EmailUtils {
	...
	// 生成邮件消息
	private static MimeMessage createMsg(Session session, String myAccount,
			User user) {
		// 创建消息对象
		...
		try {
			...
			String url = "http://" + ip + ":8080/Millet_Shopping/activate?e="
					+ Base64Utils.encode(user.getEmail()) + "&c="
					+ Base64Utils.encode(user.getActivatecode());
			// 设置正文信息
			message.setContent(user.getUsername()
					+ ",欢迎你加入我们<br>为了更好体验我们的产品，请<a href='" + url + "'>点击激活 "
